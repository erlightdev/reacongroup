...
<main id="primary" class="overflow-x-hidden" role="main">
	...
	<?php if (get_field('enable_cta_section')): ?>
		...
		if (is_array($home_cta)):
			...
			// Keep Home ACF wiring intact while matching archive CTA visual tokens.
			$cta_bg_base = '#0D6B75';
					...
					<div class="relative overflow-hidden rounded-[22px] px-5 py-10 sm:px-8 sm:py-10 lg:rounded-[24px] lg:px-12" style="background-color: <?php echo esc_attr($cta_bg_base); ?>;">
						...
						<div class="pointer-events-none absolute left-16 top-0 h-[205px] w-[1566px]" aria-hidden="true">
							<svg viewBox="0 0 1566 205" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-full w-full">
								<path d="M278.503 205L1566 -538.596L-556 -586L278.503 205Z" fill="url(#homeCtaShardLeftGradient)" fill-opacity="0.15" />
								<defs>
									<linearGradient id="homeCtaShardLeftGradient" x1="504.197" y1="170.056" x2="505.001" y2="-586" gradientUnits="userSpaceOnUse">
						...
						<div class="pointer-events-none absolute right-[-955px] h-[791px] w-[2122px]" aria-hidden="true">
							<svg width="2122" height="791" viewBox="0 0 2122 791" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-full w-full">
								<path d="M1287.5 -60L0 683.596L2122 731L1287.5 -60Z" fill="url(#homeCtaShardGradient)" fill-opacity="0.15" />
								<defs>
									<linearGradient id="homeCtaShardGradient" x1="1061.8" y1="-25.0558" x2="1061" y2="731" gradientUnits="userSpaceOnUse">
										<stop stop-color="#1ECAD3" />
										<stop offset="1" stop-color="#1ECAD3" stop-opacity="0" />
									</linearGradient>
								</defs>
						...
						<div class="relative z-10 mx-auto flex max-w-4xl flex-col items-center justify-center text-center">
							...
							<div class="mt-8 flex w-full flex-col items-center justify-center gap-3 sm:mt-9 sm:w-auto sm:flex-row sm:gap-2.5 md:mt-10">
								<?php if ($cta_primary_label && $cta_primary_url): ?>
									<a href="<?php echo esc_url($cta_primary_url); ?>" target="<?php echo esc_attr($cta_primary_target); ?>" class="group inline-flex w-full items-center justify-between gap-2.5 rounded-full bg-white py-1 pl-5 pr-1 font-sans text-base font-medium no-underline transition-all duration-300 hover:bg-white/90 sm:w-auto" style="color: <?php echo esc_attr($cta_primary_text_color); ?>;">
	...
	<?php if (get_field('enable_faq_section')): ?>
		...
		?>
			<section id="reacon-faq-section" class="w-full bg-white py-[72px] sm:py-[96px] lg:py-16" aria-labelledby="reacon-faq-heading">
				<div class="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-0">
					...
					<?php if (!empty($faq_items)): ?>
						<div class="mt-[40px] flex flex-col gap-[12px] sm:mt-[48px] lg:mt-[56px]" role="list" aria-label="<?php esc_attr_e('Frequently asked questions list', 'reacon-group'); ?>" x-data="{ activeFaq: 0 }">
							...
							?>
								<div class="reacon-faq-item rounded-[16px] border border-[#E7E7E7] px-[20px] py-[18px] transition-colors duration-300 sm:px-[24px] sm:py-[20px]" role="listitem" :class="{ 'reacon-faq-item--open': activeFaq === <?php echo esc_attr($index); ?> }">
									<button type="button" id="<?php echo esc_attr($item_id); ?>" class="flex w-full cursor-pointer items-center justify-between gap-4 rounded-md text-left outline-none focus-visible:ring-2 focus-visible:ring-[var(--reacon-teal)] focus-visible:ring-offset-2" aria-controls="<?php echo esc_attr($panel_id); ?>" :aria-expanded="activeFaq === <?php echo esc_attr($index); ?> ? 'true' : 'false'" @click="activeFaq = activeFaq === <?php echo esc_attr($index); ?> ? -1 : <?php echo esc_attr($index); ?>">
										<span class="reacon-type-h5 text-[#383B43]">
											<?php echo esc_html($question); ?>
										</span>
										<span class="reacon-type-h5 leading-none text-[#0A969B] select-none" aria-hidden="true" x-text="activeFaq === <?php echo esc_attr($index); ?> ? '−' : '+'"></span>
									</button>
							...
							<?php if ($faq_cta_title || $faq_cta_text): ?>
								<aside class="mt-[4px] rounded-[16px] bg-[#E9FBFC] px-[20px] py-[18px] sm:px-[24px] sm:py-[20px]" aria-label="<?php esc_attr_e('FAQ contact call to action', 'reacon-group'); ?>">
									<div class="flex flex-col gap-[8px]">
										<?php if ($faq_cta_title): ?><p class="reacon-type-body font-medium text-[#383B43]"><?php echo esc_html($faq_cta_title); ?></p><?php endif; ?>
									...
									<?php if ($faq_cta_link_title && $faq_cta_link_url): ?>
										<a href="<?php echo esc_url($faq_cta_link_url); ?>" target="<?php echo esc_attr($faq_cta_link_target); ?>" class="reacon-type-button flex w-full items-center justify-between gap-4 text-[#0A969B] transition-colors duration-300 hover:text-black">
